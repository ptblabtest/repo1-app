...
router.get(`${basePath}/:id/scurves`, requireAuth, async (req: any, res: any) => {
  const id = req.params.id;
  const record = await service.getScurveData(id);
  if (record) {
...
router.get(`${basePath}/:id/gantt`, requireAuth, async (req: any, res: any) => {
  const id = req.params.id;
  const record = await service.getGanttData(id);
  if (record) {
    res.json({ data: record });
  } else {
    res.status(404).json({ message: `Data not found` });
